...
const Home = () => {
  ...
  const fetchFeaturedProducts = async () => {
    try {
      ...
      // Always fetch from API first to get current database state
      const response = await axios.get('/api/products?limit=4&isFeatured=true');
      const apiProducts = response.data.products || [];
      
      if (apiProducts.length > 0) {
        setFeaturedProducts(apiProducts);
        // Update localStorage with current data
        localStorage.setItem('heroProducts', JSON.stringify(apiProducts));
      } else {
        // Clear localStorage if no products in database
        localStorage.removeItem('heroProducts');
        setFeaturedProducts([]);
      }
    } catch (error) {
      console.error('Error fetching featured products:', error);
      // Clear localStorage and show empty if API fails
      localStorage.removeItem('heroProducts');
      setFeaturedProducts([]);
    } finally {
      setLoading(false);
